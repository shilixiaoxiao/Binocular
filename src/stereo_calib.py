...
objp = np.zeros((11*15,3), np.float32)
objp[:,:2] = np.mgrid[0:15,0:11].T.reshape(-1,2)
...
images = glob.glob('../left_cali/*.jpg')
for fname in images:
...
images = glob.glob('../right_cali/*.jpg')
for fname in images:
    img = cv2.imread(fname)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    ret, corners = cv2.findChessboardCorners(gray, (15,11), None)
...
left3 = cv2.imread('../left_cali/left000.jpg')
#gray_l = cv2.cvtColor(left3, cv2.COLOR_BGR2GRAY)


dst_l = cv2.remap(left3, map1_l, map2_l, cv2.INTER_LINEAR)
cv2.imwrite('left03(rectified).jpg', dst_l)
if cv2.imwrite('left03(rectified).jpg', dst_l)==True:
    print('rectification of left camera has been done successfully.\n')
right3 = cv2.imread('../right_cali/right000.jpg')
gray_r = cv2.cvtColor(right3, cv2.COLOR_BGR2GRAY)
dst_r = cv2.remap(gray_r, map1_r, map2_r, cv2.INTER_LINEAR)
cv2.imwrite('right03(rectified).jpg', dst_r)
if cv2.imwrite('right03(rectified).jpg', dst_r) == True:
